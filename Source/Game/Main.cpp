...
INT WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ INT nCmdShow)
{
    ...
    std::shared_ptr<library::Model> myCow = std::make_shared<library::Model>("nanosuit/nanosuit.obj");
    
    if (FAILED(game->GetRenderer()->AddRenderable(L"Cow", myCow)))
    {
        return 0;
    }
    if (FAILED(game->GetRenderer()->SetVertexShaderOfRenderable(L"Cow", L"PhongShader")))
    {
        return 0;
    }
    if (FAILED(game->GetRenderer()->SetPixelShaderOfRenderable(L"Cow", L"PhongShader")))
    {
        return 0;
    }
